...
<span class="muted" style="font-size:12px">板が薄い（スプレッドが広い）銘柄をシミュから外します</span>
  ...
  <div id="simTabs" style="display:flex;gap:8px;margin-top:14px"><button class="btn" id="simTabBal" style="opacity:1">累積残高</button><button class="btn" id="simTabDay" style="opacity:.55">1日あたりの利益</button></div><div id="simTypeWrap" style="display:none;gap:8px;margin-top:8px"><button class="btn" id="simTypeB" style="opacity:1">積極なタイプ（◎○）</button><button class="btn" id="simTypeA" style="opacity:.55">慎重なタイプ（◎）</button></div><div id="simChartWrap" style="position:relative;margin-top:14px"><svg id="simChart" width="100%" height="180" style="overflow:visible;font:11px sans-serif"></svg><div id="simTip" style="display:none;position:absolute;pointer-events:none;transform:translateX(-50%);white-space:nowrap;font-size:12px;padding:4px 8px;border-radius:6px;background:var(--panel);border:1px solid var(--bd);z-index:5"></div></div><div id="simTotal" style="display:none;font-size:13px;margin-top:8px;font-weight:600"></div>
...
var __simTipKey=null;
function setSimView(v){__simView=v;if(v!=="daily"){var _st=document.getElementById("simTotal");if(_st)_st.style.display="none";var _tp=document.getElementById("simTip");if(_tp)_tp.style.display="none";__simTipKey=null;}document.getElementById("simTabBal").style.opacity=(v==="balance")?"1":".55";document.getElementById("simTabDay").style.opacity=(v==="daily")?"1":".55";document.getElementById("simTypeWrap").style.display=(v==="daily")?"flex":"none";draw();}
